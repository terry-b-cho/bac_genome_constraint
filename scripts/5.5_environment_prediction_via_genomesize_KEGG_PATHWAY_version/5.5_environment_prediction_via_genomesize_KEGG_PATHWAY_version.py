#!/usr/bin/env python3
"""
Script 5.5: Environment Prediction from KEGG Terms and Genome Size

Predict environment (one of 8 categories) using:
- KEGG functional data (reactions, KOs, or pathways)
...
sys.path.insert(0, str(Path(__file__).parent.parent / "4.5_statistical_analyses_KEGG_PATHWAY_version"))
...
# Parse arguments
parser = argparse.ArgumentParser(description='Predict environment from KEGG terms and genome size')
parser.add_argument('--data-type', type=str, required=True, choices=['reactions', 'ko', 'pathway'],
                    help='KEGG data type: reactions, ko, or pathway')
...
parser.add_argument('--output-dir', type=str, default=None,
                    help='Output directory (default: results/5.5_environment_prediction/)')
...
# Input files
MASTER_TABLE_FILE = BASE_DIR / f"results/4.5_statistical_analyses_KEGG_PATHWAY_version/02_env_cohorts/master_table_env_filtered{suffix}.tsv"
VALID_ENV_FILE = BASE_DIR / f"results/4.5_statistical_analyses_KEGG_PATHWAY_version/02_env_cohorts/valid_environments_min20{suffix}.tsv"
KEGG_LABELS_FILE = BASE_DIR / f"results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels/kegg_term_labels{suffix}.tsv"

# Output directory
if args.output_dir is None:
    OUTPUT_DIR = BASE_DIR / "results/5.5_environment_prediction"
else:
    OUTPUT_DIR = Path(args.output_dir)
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

QC_LOG_FILE = OUTPUT_DIR / f"{prefix}qc_05_env_prediction{suffix}.log"
...
log_message("=" * 80)
log_message(f"Script 5.5: Environment Prediction from KEGG {args.data_type.upper()} and Genome Size")
log_message("")
...
log_message(f"  DATA TYPE: {args.data_type}")
...
try:
    if MASTER_TABLE_FILE.exists():
        ...
        log_message(f"    Expected: {MASTER_TABLE_FILE}")
        sys.exit(1)
...
kegg_cols = filter_kegg_columns_by_prevalence(df, args.prevalence_threshold, args.data_type)
log_message(f"  Found {len(kegg_cols)} KEGG {args.data_type} columns")

# Check for missing values in KEGG columns
kegg_missing = df[kegg_cols].isna().sum().sum()
if kegg_missing > 0:
    log_message(f"  ⚠ WARNING: {kegg_missing} missing values in KEGG columns (filling with 0)")
    df[kegg_cols] = df[kegg_cols].fillna(0)

# Base features: KEGG terms + genes_total
feature_cols = kegg_cols + ['genes_total']
log_message(f"  Base features: {len(feature_cols)} ({len(kegg_cols)} KEGG {args.data_type} + genes_total)")
...
if args.normalize in ['per_gene', 'both']:
    ...
    feature_cols += [f"{col}_per_gene" for col in kegg_cols]
    log_message(f"    Added {len(kegg_cols)} per-gene normalized features")
...
if args.normalize in ['log', 'both']:
    ...
    feature_cols += ['genes_total_log'] + [f"{col}_log" for col in kegg_cols]
    log_message(f"    Added {len(kegg_cols) + 1} log-transformed features")

# Total annotated KEGG terms
total_col_name = f"{args.data_type}_total"
df[total_col_name] = df[kegg_cols].sum(axis=1)
feature_cols.append(total_col_name)
log_message(f"  Added {total_col_name} (sum of all KEGG {args.data_type} counts)")
...
# Save scaler
if HAS_JOBLIB:
    scaler_file = OUTPUT_DIR / f"{prefix}env_prediction_scaler{suffix}.pkl"
...
# ============================================================================
# Load KEGG Labels for Feature Annotation
# ============================================================================

log_message("Loading KEGG term labels for feature annotation...")
kegg_labels_map = {}
try:
    if KEGG_LABELS_FILE.exists():
        kegg_labels_df = pd.read_csv(KEGG_LABELS_FILE, sep='\t')
        kegg_labels_df['category'] = kegg_labels_df['category'].astype(str)
        # Create mapping: category -> name
        for _, row in kegg_labels_df.iterrows():
            cat = str(row['category'])
            name = row['name'] if pd.notna(row['name']) else cat
            kegg_labels_map[cat] = name
        log_message(f"  ✓ Loaded {len(kegg_labels_map)} KEGG term labels")
    else:
        log_message(f"  ⚠ KEGG labels file not found: {KEGG_LABELS_FILE}")
        log_message("    Will use numeric IDs for features")
except Exception as e:
    log_message(f"  ⚠ Error loading KEGG labels: {e}")
    log_message("    Will use numeric IDs for features")
...
def get_feature_label(feature_name):
    ...
    if feature_name == 'genes_total':
    ...
    elif feature_name == total_col_name:
        return f'Total annotated {args.data_type}'
        ...
        if base in kegg_labels_map:
            return f'{kegg_labels_map[base]} (per gene)'
        return f'{base} (per gene)'
    elif feature_name.endswith('_log'):
        base = feature_name.replace('_log', '')
        if base in kegg_labels_map:
            return f'{kegg_labels_map[base]} (log)'
        return f'{base} (log)'
    else:
        # Check if it's a KEGG term
        if feature_name in kegg_labels_map:
            return kegg_labels_map[feature_name]
        return feature_name
...
log_message("7. Feature Importance Analysis")
log_message("-" * 80)
...
if args.plot and HAS_MATPLOTLIB and len(all_feature_importances) > 0:
    ...
    try:
        ...
        if n_models == 1:
            ...
            # Single row: axes is a 1D array or list
        ...
        log_message(f"    ✓ Figure 5 saved with annotated KEGG {args.data_type} labels")
    except Exception as e:
...
# Save metrics
metrics_df = pd.DataFrame(all_metrics)
metrics_file = OUTPUT_DIR / f"{prefix}env_prediction_metrics{suffix}.tsv"
...
for model_name, cm_df in all_confusion_matrices:
    cm_file = OUTPUT_DIR / f"{prefix}env_prediction_confusion_matrix_{model_name}{suffix}.tsv"
    cm_df.to_csv(cm_file, sep='\t')
    log_message(f"  ✓ Saved confusion matrix ({model_name}) to {cm_file.name}")

# Save per-class metrics
per_class_df = pd.DataFrame(all_per_class_metrics)
per_class_file = OUTPUT_DIR / f"{prefix}env_prediction_per_class_metrics{suffix}.tsv"
...
if len(all_feature_importances) > 0:
    ...
    # Add labels if KEGG labels were loaded
    if len(kegg_labels_map) > 0:
        ...
        log_message(f"  ✓ Added KEGG {args.data_type} labels to feature importances")
    
    feature_imp_file = OUTPUT_DIR / f"{prefix}env_prediction_feature_importances{suffix}.tsv"
    feature_imp_df.to_csv(feature_imp_file, sep='\t', index=False)
    log_message(f"  ✓ Saved feature importances to {feature_imp_file.name}")

# Save models
for model_name, model in models.items():
    model_file = OUTPUT_DIR / f"{prefix}env_prediction_model_{model_name}{suffix}.pkl"
    if HAS_JOBLIB:
...
log_message("10. Robustness Checks")
log_message("-" * 80)
...
# KEGG annotation completeness
log_message(f"  KEGG {args.data_type} annotation completeness:")
for env in valid_env_list:
    env_df = df[df['environment'] == env]
    mean_kegg_total = env_df[total_col_name].mean()
    log_message(f"    {env}: {mean_kegg_total:.1f} mean total KEGG {args.data_type} counts per genome")
...
log_message("")
log_message("Script 5.5 completed successfully!")
