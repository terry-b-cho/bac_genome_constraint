...
Fit global log-log OLS scaling laws across all environments for each KEGG category.
...
# Parse arguments
parser = argparse.ArgumentParser(description='Fit global scaling laws for all KEGG categories')
parser.add_argument('--test-mode', action='store_true',
                    help='Run on small test subset (first 10 categories)')
...
OUTPUT_BASE_DIR = BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/03_global_scaling"
...
for data_type in DATA_TYPES:
    ...
    log_message("=" * 80)
    log_message(f"Script 03: Global Scaling Fits - {data_type.upper()}")
    if args.test_mode:
    ...
    # Identify KEGG category columns and filter by prevalence if needed
    kegg_cols = filter_kegg_columns_by_prevalence(df, args.prevalence_threshold, data_type)
    if args.test_mode:
        # Use first 10 categories for testing
        kegg_cols = kegg_cols[:10]
        log_message(f"  Found {len(kegg_cols)} {data_type} category columns (TEST MODE: using first 10)")
    else:
        if args.prevalence_threshold is not None:
            log_message(f"  Found {len(kegg_cols)} {data_type} category columns (filtered by {args.prevalence_threshold}%% prevalence)")
        else:
            log_message(f"  Found {len(kegg_cols)} {data_type} category columns (no prevalence filter)")
    log_message("")
    
    # Load KEGG labels if available
    ...
    if KEGG_LABELS_FILE.exists():
        try:
            ...
            log_message(f"  ✓ Loaded KEGG labels for {len(kegg_labels)} categories")
        except Exception as e:
            log_message(f"  ⚠ Could not load KEGG labels: {e}")
    ...
    def get_category_label(category_id):
        ...
        if kegg_labels is not None:
            ...
            label_row = kegg_labels[kegg_labels['category'] == cat_str]
            if len(label_row) > 0:
    ...
    output_file_tsv_name = base_name.replace('.tsv', f'{suffix}.tsv')
    output_file_tsv = f"{prefix}{output_file_tsv_name}" if prefix else output_file_tsv_name
    results_df.to_csv(OUTPUT_BASE_DIR / output_file_tsv, sep='\t', index=False)
    ...
    try:
        results_df.to_parquet(OUTPUT_BASE_DIR / output_file_parquet, engine='pyarrow')
    ...
    log_message("")
    log_message("=" * 80)
    log_message(f"Script 03 completed successfully for {data_type}!")
    log_message("=" * 80)
    log_message("")

print("")
print("=" * 80)
print("Script 03: All data types processed!")
print("=" * 80)
