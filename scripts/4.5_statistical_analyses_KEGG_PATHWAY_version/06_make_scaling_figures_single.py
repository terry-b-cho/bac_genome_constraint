...
OUTPUT_DIR = BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/06_scaling_figures"
...
# Load KEGG labels FIRST (at module level for global access)
kegg_labels_dict = {}
try:
    ...
    print(f"✓ Loaded {len(kegg_labels_dict)} KEGG labels")
except Exception as e:
    print(f"⚠ Could not load KEGG labels: {e}")
    kegg_labels_dict = {}

def get_kegg_label(category_id, max_length=50):
    """Get human-readable label, truncated if needed."""
    cat_str = str(category_id)
    label = kegg_labels_dict.get(cat_str, cat_str)
...
try:
    ...
    metabolic_terms_file = BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels" / (f"{prefix}{base_name_metabolic}" if prefix else base_name_metabolic)
    ...
    log_message(f"  ✓ Loaded {len(master_table)} genomes, {len(kegg_labels_dict)} KEGG labels")
except Exception as e:
...
try:
    ...
    try:
        full_labels = pd.read_csv(
            BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels/kegg_term_labels.tsv",
            sep='\t'
        )
        full_labels['category'] = full_labels['category'].astype(str)
        category_z_sorted['category'] = category_z_sorted['category'].astype(str)
        ...
        category_z_sorted['label'] = category_z_sorted.apply(
            lambda row: f"{row['name']} ({row})" if pd.notna(row['name']) 
            else get_kegg_label(row['category']), axis=1
        )
    except:
        category_z_sorted['label'] = category_z_sorted['category'].apply(
            lambda x: get_kegg_label(x)
        )
    ...
    ax.set_xlabel(f'{type_label} (sorted by Z_alpha_category)', fontsize=12)
    ...
    if metabolic_terms:
        ...
        if len(metabolic_category_z) > 0:
            ...
            # Merge labels
            try:
                full_labels_file = BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels" / (f"{prefix}kegg_term_labels.tsv" if prefix else "kegg_term_labels.tsv")
                full_labels = pd.read_csv(full_labels_file, sep='\t')
                full_labels['category'] = full_labels['category'].astype(str)
                metabolic_category_z_sorted['category'] = metabolic_category_z_sorted['category'].astype(str)
            ...
            type_label = type_label
            ax_met.set_xlabel(f'{type_label} - Metabolism Only (sorted by Z_alpha_category)', fontsize=12)
...
try:
    ...
    try:
        ...
        full_labels['category'] = full_labels['category'].astype(str)
        category_z_sorted_beta['category'] = category_z_sorted_beta['category'].astype(str)
    ...
    ax.set_xlabel(f'{type_label} (sorted by Z_beta_category)', fontsize=12)
...
try:
    ...
    for idx, (cat, label) in enumerate(zip(selected_cats, panel_labels)):
        ...
        cat_label = get_kegg_label(cat)  # Default
        try:
            # Try to get full_label from kegg_term_labels_for_plots.tsv
            plot_labels = pd.read_csv(
                BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels/kegg_term_labels_for_plots.tsv",
                sep='\t'
            )
            plot_labels['category'] = plot_labels['category'].astype(str)
            ...
            if len(plot_row) > 0 and 'full_label' in plot_row.columns:
                ...
                # Fallback to kegg_term_labels.tsv
                full_labels = pd.read_csv(
                    BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels/kegg_term_labels.tsv",
                    sep='\t'
                )
                ...
                if len(full_row) > 0:
                    cat_label = f"{full_row.iloc[0]['name']} ({full_row.iloc[0]})"
        except:
            pass
...
try:
    ...
    for cat_idx, cat in enumerate(selected_cats[:n_cats_to_plot]):
        ...
        for env_idx, env in enumerate(envs_to_plot):
            ...
            cat_label = get_kegg_label(cat_str)  # Default
            try:
                # Try to get full_label from kegg_term_labels_for_plots.tsv
                plot_labels_file = BASE_DIR / "results/4.5_statistical_analyses_KEGG_PATHWAY_version/05_kegg_labels" / (f"{prefix}kegg_term_labels_for_plots.tsv" if prefix else "kegg_term_labels_for_plots.tsv")
            ...
            try:
                ...
                if len(plot_row) > 0 and 'full_label' in plot_row.columns:
                    ...
                    if len(full_row) > 0:
                        ...
                        cat_label = get_kegg_label(cat_str)
            except:
                cat_label = get_kegg_label(cat_str)
...
# ============================================================================
# Panels: KEGG category count vs total annotated domains
# ============================================================================

log_message("")
log_message("Generating Panels: KEGG category count vs total annotated domains...")
try:
    # Calculate total annotated domains per genome (sum of all KEGG category counts)
